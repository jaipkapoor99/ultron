...
class RotaryEmbedding(nn.Module):
    ...
    def forward(
        ...
        return self.cos_cached[:, :, offset : seq_len + offset, :], self.sin_cached[
            :, :, offset : seq_len + offset, :
        ]
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(
        self,
        x: torch.Tensor,
        rot_emb: tuple[torch.Tensor, torch.Tensor] | None = None,
        ...
        if self.num_queries_per_kv > 1:
            ...
            k = (
                k.unsqueeze(2)
                .expand(
                    B, self.n_kv_head, self.num_queries_per_kv, k_len, self.head_dim
                )
                .reshape(B, self.n_head, k_len, self.head_dim)
            )
            v = (
                v.unsqueeze(2)
                .expand(
...
class SwiGLUMLP(nn.Module):
    ...
    def __init__(self, config: UltronConfig):
        ...
        hidden_dim = 64 * (
            (hidden_dim + 63) // 64
        )  # Round to multiple of 64 for Tensor Cores
...
class Block(nn.Module):
    ...
    def forward(
        ...
        attn_out, present_key_value = self.attn(
            self.ln_1(x), rot_emb=rot_emb, past_key_value=past_key_value
        )
        x = x + attn_out
...
class UltronModel(nn.Module):
    ...
    def __init__(self, config: UltronConfig):
        ...
        self.transformer = TransformerBody(config, self.vocab_size)
        self.lm_head = nn.Linear(config.C, self.vocab_size, bias=False)

        self.rotary_emb = RotaryEmbedding(
            config.head_dim, max_seq_len=config.T, base=config.rope_base
        )
    ...
    def forward(
        ...
        assert T + past_length <= self.config.T, (
            f"Cannot forward sequence length {T + past_length}, model block size is {self.config.T}"
        )
        ...
        for i, block in enumerate(self.transformer.h):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                x, _ = torch.utils.checkpoint.checkpoint(
                    block, x, rot_emb, None, use_reentrant=False
                )
            else:
                x, present_kv = block(x, rot_emb=rot_emb, past_key_value=past_kv)

            if use_cache and present_key_values is not None and present_kv is not None:
        ...
        if use_cache:
            return UltronOutput(
                logits=logits, loss=loss, past_key_values=present_key_values
            )
    ...
    def configure_optimizers(self, learning_rate: float):
        ...
        optimizer_adamw = torch.optim.AdamW(
            adamw_groups, lr=learning_rate, betas=(0.9, 0.95), fused=True
        )
    ...
    def generate(
        ...
        idx: torch.Tensor,
        max_new_tokens: int,
        token_selector: Callable[[torch.Tensor, torch.Tensor], torch.Tensor]
        | None = None,
        ...
        if idx.ndim != 2 or idx.size(1) == 0:
            raise ValueError(
                "idx must have shape (batch, sequence) with a non-empty sequence"
            )
        ...
        for _ in range(max_new_tokens):
            ...
            if (
                idx_next.min().item() < 0
                or idx_next.max().item() >= self.config.vocab_size
            ):
